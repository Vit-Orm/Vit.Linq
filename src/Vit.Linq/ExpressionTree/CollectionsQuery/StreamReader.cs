...
namespace Vit.Linq.ExpressionTree.CollectionsQuery
{
            ...
            cloner.clone = (node) =>
            {
                if (node?.nodeType == NodeType.Member)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(member.parameterName))
                    {
                        ...
                        valueNode = arg.GetParameter(member);
                    }
    ...
    public partial class StreamReader
    {
                ...
                case NodeType.MethodCall:
                    {
                        ...
                        switch (call.methodName)
                        {
                            ...
                            case nameof(Queryable_Extensions.ExecuteUpdate):
                                {
                                    ExpressionNode_Lambda resultSelector = call.arguments[1];
                                    switch (source)
                                    {
        ...
        ExpressionNode ReadSortField(ExpressionNode_Lambda resultSelector, CombinedStream stream)
        {
            ...
            if (sortField?.nodeType != NodeType.Member)
                throw new NotSupportedException($"[StreamReader] unexpected expression nodeType : {sortField.nodeType}");
